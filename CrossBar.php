...
class CrossBar {
	...
	**/
	...
	function getHandleFailure( $type, $id = null, $filters = array(), $account_id = null, $handler = null) {
		//$this->log(__CLASS__."->".__METHOD__." is DEPRECATED, please replace with ".__CLASS__."->get_object");
		...
		//$this->log("GET /v1/accounts/{$account_id}/$type$filter");
		$response = $this->sendHandleFailure("GET","/v1/accounts/{$account_id}/$type$filter", $handler);
	...
	function get_user_by_nameHandleFailure( $username, $account_id = null, $handler = null) {
		$response = $this->getHandleFailure('users',null,array('username'=>$username),$account_id, $handler);	
		return($response[0]);
	}
	...
	**/
	...
	function sendHandleFailure( $method, $url, $failureHandler, $post_data = NULL, $type = 'application/json', $accept_type = "application/json, application/octet-stream, audio/*, */*") {
		$bldred=''; 
		$bldgrn=''; 
		$bldylw='';
		$bldblu='';
		...
		// Allow the failure handler to correct the credentials and try again
		if(stristr($this->headers, "401 Unauthorized") && isset($failureHandler)) {
			$failureHandler->fix();
			$this->xauth = $failureHandler->newXauth;
			return $this->sendHandleFailure($method, $url, null, $post_data, $type, $accept_type);
		}
